...
(defcustom unicode-math-input-deterministic nil
  "When nil, allows selection of candidate translations with numeric keys.
...
(defcustom unicode-math-input-insert-tex nil
  "If t, `unicode-math-input' inserts TeX commands rather than the Unicode characters."
  :type 'boolean)
(make-variable-buffer-local 'unicode-math-input-insert-tex)
...
(defun unicode-math-input--format (it)
  "Generate candidates for `unicode-math-input'.
IT is an entry of `unicode-math-input-symbols'."
  (format "\\%-25s %s %s"
          (car it)
          (propertize (char-to-string (cdr it)) 'face 'font-lock-keyword-face)
          (get-char-code-property (cdr it) 'name)))

(defun unicode-math-input--insert (it)
  "Insert selection of `unicode-math-input' at point.
IT is a string generated by `unicode-math-input--format'."
  (let ((tex (save-match-data
               (string-match "\\\\\\([A-Za-z]+\\)" it)
               (match-string 1 it))))
    (insert (if unicode-math-input-insert-tex
                (concat "\\" tex)
              (alist-get tex unicode-math-input-symbols "" nil 'string-equal)))))

;;;###autoload
(defun unicode-math-input ()
  "Browse and insert Unicode mathematical symbols."
  (interactive)
  (unicode-math-input--insert
   (completing-read
    "Math symbol: "
    (mapcar 'unicode-math-input--format unicode-math-input-symbols)
    nil
    t)))
