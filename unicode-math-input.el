...
(dolist (it unicode-math-input-symbols)
  (dolist (n (number-sequence (min unicode-math-input-min-prefix (length (car it)))
                              (length (car it))))
    (quail-defrule (concat unicode-math-input-escape (substring (car it) 0 n)) (cadr it)
                   "unicode-math" t)))
...
(setq unicode-math-browse-format-function
      (lambda (tex char)
        (format "\\%-25s %s %s"
                tex
                (propertize (char-to-string char) 'face 'font-lock-keyword-face)
